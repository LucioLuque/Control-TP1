...
Dado que valores adecuados requieren $\text{MG} > 6\ \si{dB}$ y $\text{MF} \approx 30^\circ$--$60^\circ$, el sistema ya posee márgenes holgados. Por lo tanto, \textit{no es estrictamente necesario introducir un compensador por motivos de estabilidad}.
...
\begin{tabular}{lcc}
  ...
  \midrule
  Tiempo de establecimiento ($t_s$, 2\%) & $1.683\ \si{s}$ & $\leq 7.5\ \si{s}$ \checkmark \\
  Sobrepico máximo ($M_p$)               & $14.633\ \%$     & $\leq 15\ \%$ \checkmark \\
...
\[
  e_{ss} = \lim_{s\to 0} s\!\left[\frac{1}{s} - \frac{\Theta}{V}\frac{1}{s} - \frac{\Theta}{N}\frac{0.2}{s}\right]
  = 1 - 1 - 0.2 = 0.2 \quad (20\%)
\]
...
Se cumplen el tiempo de establecimiento y máximo sobrepico, pero el error es mucho mayor que el especificado. Es por esto que se plantea el uso de un controlador PI de la forma:
\[
  C(s) = K_p + \frac{K_i}{s}
\]
...
\begin{align*}
  \theta &= \bigl[(V(s)-\theta)C(s) + N(s)\bigr]G(s) \\
  (1 + G(s)C(s))\,\theta &= G(s)C(s)\,V(s) + G(s)\,N(s) \\[4pt]
...
\textbf{Demostración de $e_{ss}=0$.}
Con referencia escalón unitario y perturbación constante de magnitud $0.2$:
...
\[
  \lim_{s\to 0}\frac{\Theta}{V} = \frac{0.25K_i}{0.25K_i} = 1, \qquad
  \lim_{s\to 0}\frac{\Theta}{N} = \frac{0}{0.25K_i} = 0 \quad (K_i \neq 0)
\]

\[
  \therefore \quad e_{ss} = 1 - 1 - 0 = 0
\]

\textbf{Especificaciones sobre $\zeta$ y $\omega_n$.}
Para aproximar el sistema de orden 4 como uno de orden 2 con polos dominantes, las restricciones son:
...
\[
  e_{ss} = \lim_{s\to 0}\frac{1}{s}\!\left[1 - \frac{\Theta(s)}{V(s)}\right]
  = \lim_{s\to 0}\frac{0.003s^3+0.0515s^2+0.2s}{0.003s^4+\cdots+0.25K_i} = 0
\]

Este resultado era esperable: el controlador PI convierte al sistema en \textbf{Tipo~2}, lo que garantiza seguimiento con error nulo ante entradas rampa.
...
\textbf{¿Por qué es posible usar este compensador?} El compensador lead normalizado tiene ganancia unitaria en CC, por lo que no modifica la ganancia de lazo en $s=0$. Dado que el sistema nominal ya es de Tipo~1 (tiene un integrador), el error de posición ante una entrada escalón sigue siendo nulo tras la compensación.
...
\begin{itemize}
  ...
  \item \textbf{Reducción del ancho de banda en lazo cerrado:} paradójicamente, el mayor amortiguamiento reduce el ancho de banda (de $3.72\ \si{rad/s}$ para MF = 35\ensuremath{^\circ} a $2.36\ \si{rad/s}$ para MF = 65\ensuremath{^\circ}), lo que se traduce en una respuesta más lenta ante referencias.
  ...
  \item \textbf{Ganancia alta de alta frecuencia:} el factor $K_c = 1/\alpha \approx 6.44$ amplifica las señales de alta frecuencia, incrementando la sensibilidad del sistema al ruido en la medición.
\end{itemize}
